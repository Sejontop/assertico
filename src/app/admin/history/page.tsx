import { listAllHistory, listUsersForFilter } from "@/lib/admin";
...
export default async function AdminHistoryPage({ searchParams }: AdminHistoryPageProps) {
  ...
  const [result, users] = await Promise.all([
    listAllHistory({ userId, page, pageSize: PAGE_SIZE }),
    listUsersForFilter()
  ]);
  ...
  return (
    <div>
      <div className="border-b border-border px-6 py-4">
        ...
        <p className="text-sm text-muted-foreground">{result.total} executions.</p>
      </div>
      <div className="space-y-4 p-6">
        <form action="/admin/history" method="GET" className="flex items-end gap-3">
        ...
        <AdminHistoryTable entries={result.entries} />
